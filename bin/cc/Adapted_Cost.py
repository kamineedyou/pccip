...
# Get all the misaligned transitions so that later we can check whether they are in other net fragments
    ...
    for trace in aligned_traces:
        ...
        if trace['fitness'] != 1:
            for alig in trace['alignment']:
                if Get_Acti(alig) is not None and Get_Acti(alig) not in unfited_traces:
